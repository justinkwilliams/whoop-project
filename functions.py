...
def add_day_of_week(df: pd.DataFrame, date_col: pd.Series):
    """Converts a date column to datetime object and creates a column with the day of the week from a date column in the same row.

    Args:
        df (pd.DataFrame): DataFrame containing the date column.
        date_col (pd.Series): Column containing the date.
    Returns:
        pd.DataFrame: Updated DataFrame with a column for the day of the week for each row.
    """
    df[date_col] = pd.to_datetime(df[date_col])
    df['Day of Week'] = (df[date_col].dt.day_name).astype('category')
    return df
